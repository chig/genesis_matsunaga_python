...
module rmsd_impl_mod
  ...
  public  :: analyze_zerocopy_full_with_fitting
...
!      call error_msg('Analyze> mass weighted is not allowed')
  ...
  !> @brief        RMSD analysis with full zero-copy (pre-allocated result array)
  !! @authors      Claude Code
  !! @param[in]    mass           : mass array pointer (view of Python NumPy)
  !! @param[in]    ref_coord      : reference coordinates (3, n_atoms)
  !! @param[in]    trajes_c       : trajectory C structure
  ...
  subroutine analyze_zerocopy_full(mass, ref_coord, trajes_c, ana_period, &
                                   analysis_idx, n_analysis, mass_weighted, &
                                   rmsd_results, nstru_out)
    use s_trajectories_c_mod
    ...
    ! Output summary
    write(MsgOut,'(A)') ''
    write(MsgOut,'(A)') 'Analyze_zerocopy_full> RMSD analysis completed (full zero-copy, no fitting)'
    write(MsgOut,'(A)') ''
  ...
  !======1=========2=========3=========4=========5=========6=========7=========8
  !
  !  Subroutine    analyze_zerocopy_full_with_fitting
  ...
  subroutine analyze_zerocopy_full_with_fitting(mass, ref_coord, n_atoms, &
                              trajes_c, ana_period, &
                              fitting_idx, n_fitting, &
                              analysis_idx, n_analysis, &
                              fitting_method, mass_weighted, &
    ...
    ! Output summary
    write(MsgOut,'(A)') ''
    write(MsgOut,'(A)') 'Analyze_zerocopy_full_with_fitting> RMSD analysis completed (full zero-copy, with fitting)'
    write(MsgOut,'(A)') ''

    return

  end subroutine analyze_zerocopy_full_with_fitting
