...
module diffusion_c_mod
  ...
  public :: diffusion_analysis_zerocopy_full_c
...
contains
  ...
  !======1=========2=========3=========4=========5=========6=========7=========8
  !
  !  Subroutine    diffusion_analysis_zerocopy_full_c
  ...
  subroutine diffusion_analysis_zerocopy_full_c(msd_ptr, ndata, ncols, &
                                                time_step, distance_unit, ndofs, &
                                                start_step, stop_step, &
                                                out_data_ptr, out_data_size, &
                                                diff_coeff_ptr, n_sets, &
                                                status, msg, msglen) &
        bind(C, name="diffusion_analysis_zerocopy_full_c")
    use conv_f_c_util
    implicit none
    ...
    if (.not. c_associated(msd_ptr)) then
      call error_set(err, ERROR_INVALID_PARAM, &
                     "diffusion_analysis_zerocopy_full_c: msd_ptr is null")
      call error_to_c(err, status, msg, msglen)
      return
    end if
    ...
    if (ndata <= 0) then
      call error_set(err, ERROR_INVALID_PARAM, &
                     "diffusion_analysis_zerocopy_full_c: ndata must be positive")
      call error_to_c(err, status, msg, msglen)
      return
    end if
    ...
    if (ncols < 2) then
      call error_set(err, ERROR_INVALID_PARAM, &
                     "diffusion_analysis_zerocopy_full_c: ncols must be >= 2")
      call error_to_c(err, status, msg, msglen)
      return
    end if
    ...
    if (.not. c_associated(out_data_ptr)) then
      call error_set(err, ERROR_INVALID_PARAM, &
                     "diffusion_analysis_zerocopy_full_c: out_data_ptr is null")
      call error_to_c(err, status, msg, msglen)
      return
    end if
    ...
    if (.not. c_associated(diff_coeff_ptr)) then
      call error_set(err, ERROR_INVALID_PARAM, &
                     "diffusion_analysis_zerocopy_full_c: diff_coeff_ptr is null")
      call error_to_c(err, status, msg, msglen)
      return
    end if
    ...
    ! Run analysis
    write(MsgOut,'(A)') '[STEP1] Diffusion Analysis (full zerocopy interface)'
    write(MsgOut,'(A)') ' '

    call analyze_zerocopy_full(msd_f, ndata, ncols, &
                               time_step, distance_unit, ndofs, &
                               start_step, stop_step, &
                               out_data_f, diff_coeff_f)

  end subroutine diffusion_analysis_zerocopy_full_c
