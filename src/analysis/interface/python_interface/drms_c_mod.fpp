...
module drms_c_mod
  ...
  public :: drms_analysis_zerocopy_full_c
...
contains
  ...
  !======1=========2=========3=========4=========5=========6=========7=========8
  !
  !  Subroutine    drms_analysis_zerocopy_full_c
  !> @brief        DRMS analysis with full zero-copy (pre-allocated result)
  !! @authors      Claude Code
  !! @param[in]    contact_list_ptr : pointer to contact atom pairs (2, n_contact)
  !! @param[in]    contact_dist_ptr : pointer to reference distances
  !! @param[in]    n_contact        : number of contacts
  ...
  subroutine drms_analysis_zerocopy_full_c(contact_list_ptr, contact_dist_ptr, &
                                           n_contact, s_trajes_c, ana_period, &
                                           pbc_correct, result_ptr, result_size, &
                                           nstru_out, status, msg, msglen) &
        bind(C, name="drms_analysis_zerocopy_full_c")
    implicit none
    ...
    if (n_contact <= 0) then
      call error_set(err, ERROR_INVALID_PARAM, &
                     "drms_analysis_zerocopy_full_c: n_contact must be positive")
      call error_to_c(err, status, msg, msglen)
      return
    end if
    ...
    if (.not. c_associated(contact_list_ptr)) then
      call error_set(err, ERROR_INVALID_PARAM, &
                     "drms_analysis_zerocopy_full_c: contact_list_ptr is null")
      call error_to_c(err, status, msg, msglen)
      return
    end if
    ...
    if (.not. c_associated(contact_dist_ptr)) then
      call error_set(err, ERROR_INVALID_PARAM, &
                     "drms_analysis_zerocopy_full_c: contact_dist_ptr is null")
      call error_to_c(err, status, msg, msglen)
      return
    end if
    ...
    if (.not. c_associated(result_ptr)) then
      call error_set(err, ERROR_INVALID_PARAM, &
                     "drms_analysis_zerocopy_full_c: result_ptr is null")
      call error_to_c(err, status, msg, msglen)
      return
    end if
    ...
    if (result_size <= 0) then
      call error_set(err, ERROR_INVALID_PARAM, &
                     "drms_analysis_zerocopy_full_c: result_size must be positive")
      call error_to_c(err, status, msg, msglen)
      return
    end if
    ...
    ! Run analysis
    write(MsgOut,'(A)') '[STEP1] DRMS Analysis (full zero-copy interface)'
    write(MsgOut,'(A)') ' '

    call analyze_zerocopy_full(contact_list_f, contact_dist_f, n_contact, &
                               s_trajes_c, ana_period, pbc_flag, &
                               result_f, nstru_local)

    nstru_out = nstru_local

  end subroutine drms_analysis_zerocopy_full_c
